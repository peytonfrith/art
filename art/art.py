...
def art(artname, number=1, space=1, __detailed_return=False):
    ...
    if artname in ["random", "rand", "rnd"]:
        ...
        artname = secrets.choice(filtered_arts)
...
def wizard_font(text):
    ...
    if text_length <= TEXT_XLARGE_THRESHOLD:
        font = secrets.choice(XLARGE_WIZARD_FONT)
    elif text_length <= TEXT_LARGE_THRESHOLD:
        font = secrets.choice(LARGE_WIZARD_FONT)
    elif text_length <= TEXT_MEDIUM_THRESHOLD:
        font = secrets.choice(MEDIUM_WIZARD_FONT)
    else:
        font = secrets.choice(SMALL_WIZARD_FONT)
    return font
...
def indirect_decoration(decoration):
    ...
    if decoration in ["random", "rand", "rnd"]:
        decoration = secrets.choice(decorations)
        return decoration
...
def mix_letters():
    ...
    for i in letters:
        random_font = secrets.choice(fonts)
